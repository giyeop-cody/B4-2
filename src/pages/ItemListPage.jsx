      ...
      <StateView loading={loading} error={error} data={items} emptyMessage="표시할 데이터가 없습니다." emptyActionLabel="첫 아이템 등록하기" emptyActionTo="/items/new" onRetry={refetch}>
        {(data) => (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {data.map(item => <ItemCard key={item.id} item={item} onDelete={setConfirmId} />)}
          </div>
        )}
      </StateView>
      <ConfirmDialog open={confirmId !== null} message="정말 삭제하시겠습니까?" onConfirm={handleDelete} onCancel={() => setConfirmId(null)} />
    </div>
  )
}
