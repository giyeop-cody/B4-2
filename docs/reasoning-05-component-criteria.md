...
| 9 | DataSourceBadge | 현재 저장 위치 표시 | 원격/로컬/누락 상태를 같은 모양으로 표시 | Layout |
| 10 | CategoryFilter | 카테고리와 개수 선택 | 필터 입력 책임 분리 | ItemListPage |
| 11 | ProtectedRoute | 로그인 여부에 따른 화면 보호 | 인증 분기 중복 방지 | ProfilePage 라우트 |
