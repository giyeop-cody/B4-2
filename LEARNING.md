...
1. Supabase Dashboard에서 RLS SQL 검토·실행
2. 익명 SELECT 성공과 익명 INSERT/UPDATE/DELETE 거절 확인
3. 확인된 계정으로 로그인 CRUD 확인
4. main 병합과 Vercel 재배포 뒤 익명·로그인 화면 검사

실행하지 않은 외부 작업은 문서 파일이 있다는 이유만으로 완료라고 적지 않는다.
